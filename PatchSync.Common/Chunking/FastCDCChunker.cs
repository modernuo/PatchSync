...
/// Port of https://github.com/jotfs/fastcdc-go
///
/// FastCDC uses a Gear hash with normalized chunking to produce uniform
/// chunk size distribution around the target average size.
///
...
public sealed class FastCDCChunker : IChunker
{
    ...
    public IEnumerable<ChunkBoundary> Chunk(Stream input, ChunkingOptions options)
    {
        ...
        // Must match fastcdc-go: bits = round(log2(avgSize))
        int bits = (int)Math.Round(Math.Log2(options.AverageSize));
        int smallBits = bits + options.Normalization;
        int largeBits = bits - options.Normalization;

        ulong maskS = (1UL << smallBits) - 1; // Strict mask (below average)
        ulong maskL = (1UL << largeBits) - 1; // Loose mask (above average)

        // Buffer size needs to be at least maxSize to ensure we can always
        // find a boundary. Using 2x maxSize allows efficient buffer reuse.
        int bufferSize = options.MaxSize * 2;
        byte[] buffer = new byte[bufferSize];
        int dataStart = 0;  // Start of valid data in buffer
        int dataEnd = 0;    // End of valid data in buffer (exclusive)
        long streamOffset = 0;
        bool eof = false;
        ...
        while (true)
        {
            int available = dataEnd - dataStart;

            // Fill buffer if we don't have enough data for a max-size chunk
            if (available < options.MaxSize && !eof)
            {
                // Compact buffer: move remaining data to start
                if (dataStart > 0 && available > 0)
                {
                    Buffer.BlockCopy(buffer, dataStart, buffer, 0, available);
                }
                dataStart = 0;
                dataEnd = available;

                // Read more data into buffer
                int toRead = bufferSize - dataEnd;
                int bytesRead = input.Read(buffer, dataEnd, toRead);
                if (bytesRead == 0)
                {
                    eof = true;
                }
            ...
            available = dataEnd - dataStart;
            if (available == 0)
            {
                yield break;
            }

            // Find next chunk boundary using Span for zero-copy
            ReadOnlySpan<byte> data = buffer.AsSpan(dataStart, available);
            int chunkLength = FindBoundary(data, options.MinSize, options.AverageSize, options.MaxSize, maskS, maskL);

            // Compute SHA256 hash using incremental API (reuses state)
            sha256.AppendData(buffer, dataStart, chunkLength);
            Span<byte> hashBuffer = stackalloc byte[32];
            sha256.GetHashAndReset(hashBuffer);
            byte[] hash = hashBuffer.ToArray();

            yield return new ChunkBoundary(streamOffset, chunkLength, hash);
    ...
    /// <summary>
    /// Finds the next chunk boundary in the data using FastCDC algorithm.
    /// </summary>
        ...
        ulong maskL)
    {
        ...
        // If data is smaller than min size, return all of it
        if (length <= minSize)
        {
            return length;
        }
        ...
        // Phase 1: From minSize to avgSize, use strict mask (harder to find boundary)
        // This prevents too many small chunks
        int normEnd = Math.Min(n, avgSize);
        for (int i = minSize; i < normEnd; i++)
        {
        ...
        // Phase 2: From avgSize to maxSize, use loose mask (easier to find boundary)
        // This prevents too many large chunks
        for (int i = normEnd; i < n; i++)
        {
            fingerprint = (fingerprint << 1) + GearTable[data[i]];
        ...
        // No boundary found, force split at max size (or end of data)
        return n;
    }

    /// <summary>
    /// Gear hash lookup table - 256 random 64-bit values.
    /// MUST match fastcdc-go exactly for compatible chunk boundaries.
    /// </summary>
    private static readonly ulong[] GearTable =
    [
